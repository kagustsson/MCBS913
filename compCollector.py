...
dir_list = "_dir_list"  # all comparison dir is round_dir+dir_list
round_dir = "round"     # round dirname = "round" + number, eg,"round1"
result_dir_prefix = "Results"

# 1 output csv files for all rounds
round_csv = csv.writer(open('all_round_comparsion_result.csv', "w"),
                       lineterminator='\n')
round_csv.writerow(["file name", "total expected", "num of assigned genes",
                    "num of unassigned genes", "ratio of assigned genes"])
...
for i in range(0, TotalRound):
    print("============= round " + str(i + 1) + " =============")
    r1_dir_file = open(round_dir_lists[i], 'r')
    dir_list = r1_dir_file.read().splitlines()

    # count for processing
    count = 0
    totalfiles = subprocess.check_output(
        "ls -l " + round_dirs[i] + "| wc -l", shell=True).replace('\n', '')
    ...
    for line in dir_list:
        ...
        output1 = subprocess.check_output(
            "grep -c '>' " + ort_faa1, shell=True)
        output2 = subprocess.check_output(
            "grep -c '>' " + ort_faa2, shell=True)
        ...
        if total_expected == num_ass_gene + num_uss_gene:
            ratio = (float)(num_ass_gene / (total_expected * 1.0))
            round_csv.writerow(
                [line, total_expected, num_ass_gene, num_uss_gene, ratio])
        else:
            print("-------------------------------------")
